...
export function BarcodeScanner({ onDetected }: BarcodeScannerProps) {
  ...
  const startCameraScanner = useCallback(async () => {
    ...
    if (!scannerRef.current) {
      setError("スキャナーの初期化に失敗しました。ページを再読み込みしてお試しください。");
      setIsScanning(false);
      setScanMode(null);
      return;
    }
    ...
    try {
      ...
      try {
        ...
        throw new Error("カメラへのアクセスが拒否されました。ブラウザの設定でカメラのアクセスを許可してください。");
      }
      await Quagga.init({
        inputStream: {
          name: "Live",
          ...
          {detectedCode && (
            <div className="mt-4 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
              <p className="text-green-800 dark:text-green-200">
                検出されたコード: <span className="font-mono font-bold">{detectedCode}</span>
              </p>
            </div>
          )}
        </div>
